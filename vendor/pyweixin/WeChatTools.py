...
class Navigator():
    ...
    def open_weixin(is_maximize:bool=None,window_size:tuple=None)->(WindowSpecification|None):
        ...
        real_class=main_window.class_name()
        # 本地补丁：4.1.10/4.1.13 客户端顶层窗口的 win32 类名是 Qt51514QWindowIcon
        #（UIA ClassName 视窗口状态可能为 mmui::MainWindow），两种都接受
        if real_class not in ('mmui::MainWindow','mmui::LoginWindow','Qt51514QWindowIcon'):
            raise NotFoundError
        if real_class=='mmui::LoginWindow':
            raise NotLoginError
        main_window.restore()
